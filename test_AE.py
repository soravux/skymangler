...
def genDataset(folder, n=-1):
    ...
    for fname in fichiers:
        if not 'envmap.exr.mat' in fname:
            continue
        if i == n:
            break
        i += 1

        dataRaw = scipy.io.loadmat(os.path.join(folder, fname))
        ...
        #print(dfilter.shape, dgray.shape)

        # Ponderation plutot arbitraire...
        ...
        if D is None:
            ...
            assert D == len(dfilter), "Dimensions size mismatch : {} / {}".format(D, len(dfilter))
...
def test_dA(learning_rate=0.1, training_epochs=15,
    ...
    # LES VARIABLES DU TRAIN SET DOIVENT ETRE SHARED (VOIR logistic_sgd)
    ...
    # Ne fonctionne pas, parce la taille n'est pas proche d'un nombre carre...
    ...
    # SUPPRIMER CE EXIT SI ON VEUT CONTINUER
    exit()
